...
#
# This script extracts file-dependencies from TeX source files
# (*.tex) and produces output in a Makefile format suitable for
...
# re-running this script) whenever `foo.pdf' is generated, e.g.
#
#   %.pdf: %.tex
...
#
# Usage: extract_filenames <texfile> <command>
#
extract_filenames() {
...
# variable.
#
# Usage: filter_filelist <dir> <filelist> <ext1> <ext2> ...
#
filter_filelist() {
    local d fs f x
...
while [ -n "${tocheck}" ]; do
    ...
    for file in ${tocheck}; do
        ...
        bibs="${files}"
        extract_filenames "${file}" 'externaldocument' # Cross references
        extdocs="${files}"
        ...
        for dir in "" "${texdir}/"; do
            ...
            filter_filelist "${dir}" "${extdocs}" "" ".aux" ".tex"
            deps="${deps}${res}"
        done
    done
